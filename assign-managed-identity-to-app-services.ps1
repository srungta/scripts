...
Write-Host '...Hi...'
Write-Host 'You are using a script to assign managed identity to app services'
...
if ([string]::IsNullOrWhiteSpace($PathToJson)) {
    Write-Host "Using default value 'templates/assign-managed-identity-to-app-services.json'."
    $PathToJson = './templates/assign-managed-identity-to-app-services.json'
} 
...
foreach ($appService in $content) {
    Try {
        Write-Host 'Activating identity in ' $appService.appServiceName
        Set-AzureRmWebApp  -AssignIdentity $true -Name $appService.appServiceName -ResourceGroupName $appService.appServiceResourceGroup
    }
    Catch {
        Write-Host 'Could not create application '
        Write-Host  $appService.appServiceName
