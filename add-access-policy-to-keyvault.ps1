...
Write-Host '...Hi...'
Write-Host 'You are using a script to add service principals to multiple aad applications quickly'
...
if ([string]::IsNullOrWhiteSpace($PathToJson)) {
    Write-Host "Using default value 'templates/add-access-policy-to-keyvault.json'."
    $PathToJson = './templates/add-access-policy-to-keyvault.json'
} 
...
foreach ($application in $content) {
    Write-Host 'Adding' $application.applicationName ' service principal to keyvault ' $application.keyvaultName
    $ExistingApplication = Get-AzureRmADServicePrincipal  -ApplicationId $application.applicationId
    Set-AzureRmKeyVaultAccessPolicy -VaultName $application.keyvaultName -PermissionsToSecrets get, list -ObjectId $ExistingApplication.Id
}