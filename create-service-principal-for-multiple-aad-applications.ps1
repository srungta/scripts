...
if ([string]::IsNullOrWhiteSpace($PathToJson)) {
    Write-Host "Using default value 'templates/create-service-principal-for-multiple-aad-applications.json'."
    $PathToJson = './templates/create-service-principal-for-multiple-aad-applications.json'
} 
...
foreach ($application in $content) {
    Try {
        Write-Host 'Adding servcie principal to application' $application.applicationName
